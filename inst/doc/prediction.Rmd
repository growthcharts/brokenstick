...
The plot of the observed versus predicted is now considerably tighter, and the overall fit appears to be quite high. The correlation between the observed and predicted scores using the raw scale is equal to `r round(cor(smc$yhatcm, smc$lencm, use = "pairwise"), 4)`. The standard deviation of the residuals in the cm scale is equal to `r round(sd(smc$yhatcm - smc$lencm, na.rm = TRUE), 3)`, which is very small. 
...
In terms of the raw meassurement scale we find a correlation between the observed and predicted scores equal to `r round(cor(smc$yhatcm, smc$lencm, use = "pairwise"), 4)`. The standard deviation of the residuals in the cm scale is equal to `r round(sd(smc$yhatcm - smc$lencm, na.rm = TRUE), 4)`, about the size of the measurement error for height.
