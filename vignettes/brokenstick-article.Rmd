...
Many R packages offer tools for interpolation. The core \pkg{splines} package and the \pkg{akima} package [@akima2021] contains classic interpolation methods for one- and two-dimensional smoothing. Most contributed packages concentrate on time series or spatial interpolation, e.g. \pkg{deldir}, \pkg{interp}, \pkg{fields}, \pkg{gapfill}, \pkg{geoR}, \pkg{gstat} and \pkg{spatial}. See @li2014 and @lepot2017 for overviews of the different concepts and methodologies. Most interpolation techniques rely on neighbouring information, in time, space or both. The broken stick model addresses the problem where many independent replications provide short irregular multivariate time series, say of 5-30 time points. The scientific interest is to dynamically predict and update future observations. The model applies the linear mixed model to increase stability for such series by borrowing information across replicates. As there are no satisfactory solutions to this problem, the \pkg{brokenstick} package intends to fill this gap.

Substantive researchers often favour repeated measures over the use of linear mixed models because of their simplicity. For example, we can easily fit a subject-level model to predict future outcomes conditional on earlier data with repeated measures data. While such simple regression models may be less efficient than modelling the complete data [@diggle2002, Sec. 6.1], increased insight may be more valuable than increased precision.
...
The original motivation for developing the broken stick model was to facilitate the statistical analysis and testing of critical ages in the onset of childhood obesity [@dekroon2010], with extensions to multiple imputation [@vanbuuren2018]. There is good support in R for fitting child growth data. We mention some related approaches. Methods for estimating growth references with parametric models are `gamlss()` from \pkg{gamlss} [@stasinopoulos2007] and its Bayesian incarnation `bamlss()` from \pkg{bamlss} [@umlauf2019]. Nonparametric alternatives that estimate quantiles directly are `rq()` from \pkg{quantreg} [@koenker2018] and `expectreg.ls()` from \pkg{expectreg} [@otto-sobotka2021]. Methods for modelling and smoothing growth curves fit trajectories per child include `smooth.basisPar()` from \pkg{fda} [@ramsay2021], `gam()` from \pkg{mgcv} [@wood2011], `loess()` and `smooth.spline()` from base \pkg{stats} [@r2020]. Models that smooth by borrowing strength across children are `face.sparse()` from \pkg{face} [@xiao2021], `lmer()` from \pkg{lme4} [@bates2015], and `sitar()` from \pkg{sitar} [@cole2021]. The broken stick model fits in the latter tradition, and features an intuitive parametrisation of each individual growth curves by a series of connected straight lines. See @anderson2019 for an overview and comparison of these methods.

The present paper highlights various computational tools from the \pkg{brokenstick} v 2.0.0 package. The package contains tools to fit the broken stick model to data, export the fitted model's parameters, create imputed values of the model, and predict broken stick estimates for new data. Also, the text illustrates how the tool helps to solve various analytic problems.
...
Estimation of the broken stick model relies on two well-developed \proglang{R} functions: `splines::bs()` [@r2020] and `lme4::lmer()`.[@bates2015] Let us first look at the individual steps in the calculation. The following snippet calculates the matrix of $B$-splines for the time variable `age`:
...
```{r}
library(lme4)
ctl <- lmerControl(check.conv.grad = .makeCC("warning", tol = 8e-3))
f <- hgt_z ~ 0 + age_0 + age_0.5 + age_1 + age_2 + age_3 +
  (0 + age_0 + age_0.5 + age_1 + age_2 + age_3 | id)
mod <- lmer(f, data, control = ctl)
```

The formula removes the intercept and specifies each knot as a random effect with `id` as grouping factor. The `control` argument suppresses the warning `Model failed to converge with max|grad| = 0.0066811 (tol = 0.002, component 1)`. Although such warnings often occur I found that the broken estimates generally look sound and reasonable. Note however that my experience derives primarily from child growth data, so there is no guarantee that this apparent robustness will hold for other types of data. Warnings become less frequent for a lower number of knots and for larger samples sizes.

Step three converts the result into brokenstick estimates by summing the fixed and random effects, effectively forming fitted trajectories for all persons.

```{r}
bse <- t(t(ranef(mod)$id) + fixef(mod))
head(round(bse, 3), 3)
```
...
The `brokenstick::brokenstick()` function performs these steps in one call:

```{r}
fit1 <- brokenstick(hgt_z ~ age | id, data = smocc_200, method = "lmer",
                    knots = c(0, 0.5, 1, 2), boundary = c(0, 3),
                    control = ctl)
head(predict(fit1, x = "knots", shape = "wide"), 3)
```

We may use `plot()`, `predict()` and `summary` methods for `fit1`, or inspect the fitted linear mixed model at `fit1$mod` in detail by standard tools in the \pkg{lme4} package.

### Parameter estimation, method kr

The calculation time of `lme4::lmer()` rapidly increases with the number of random effects. More than ten random effects (knots) takes significant time, and beyond 15 knots is generally impossible to fit. The \pkg{brokenstick} package provides another alternative, the *Kasim-Raudenbush (KR) sampler* [@kasim1998]. This method simulates draws from the posterior distributions of parameters from a two-level normal model with heterogeneous within-subject variances. The speed of the KR-Raudenbush sampler is almost insensitive to the number of random effects and depends primarily on the *total number of iterations* and somewhat on sample size. A call to `control_kr()` produces a list  of defaults that should be reasonable across a wide range of cases. 

The `kr` method is the default estimation method in the \pkg{brokenstick} package.

```{r}
fit2 <- brokenstick(hgt_z ~ age | id, data = smocc_200, 
                    knots = c(0, 0.5, 1, 2), boundary = c(0, 3),
                    seed = 12321)
head(predict(fit2, x = "knots", shape = "wide"), 3)
```

In general, the `lmer` and `kr` methods produces broken stick estimates that are similar, but not identical. Substantial discrepancies may occur in areas where data are sparse, for example, at the right boundary knot. The behaviour of the `kr` method has not been studied as well as `lmer`, so it is more experimental. 

Apart from being faster for cases with many knots, the KR-sampler opens up interesting analytic options:
...
### Broken stick estimation for new data

Apart from parameter estimates of the broken stick model, we also need a way to estimate broken stick values for a given set of parameters and (new) user data. The `brokenstick:::EB()` function implements the empirical Bayes (EB) estimate, also known as BLUP (@skrondal2009, p. 683). The procedure is the workhorse underlying the more versatile `predict()` method.
...
`kr()`                 | Kasim-Raudenbush sampler for two-level model (internal)
...
It is easy to convert the measurements into the $Z$-score scale, fit the model, and convert back to the raw scale afterwards, if desired. There are several \proglang{R} packages that assist in the calculations: \pkg{AGD} [@vanbuuren2018b], \pkg{anthro} [@schumacher2020], \pkg{childsds} [@vogel2020], \pkg{growthstandards} [@hafen2021], \pkg{nlreferences} [@vanbuuren2021] and \pkg{zscorer} [@myatt2019].

The `smocc_200` data contains the height measurement both in the original scale in cm (`hgt`) and the $Z$-score scale (`hgt_z`) relative to the height references from the Fourth Dutch Growth study [@fredriks2000]. For illustration, let us recalculate height SDS using the \pkg{nlreferences} package.

```{r eval=FALSE}
remotes::install_github("growthcharts/nlreferences")
library(nlreferences)
hgt_z <- transform2z(smocc_200, ynames = "hgt")
identical(pull(hgt_z), smocc_200$hgt_z)
```
...
Figure 3 shows that, as expected, the empirical Z-score distribution is close to the standard normal. The next section concentrate on modelling `hgt_z`.

Function `transform2y()` applies the inverse transformation of $Z$-scores to the original scale. The following snippet converts `hgt_z` into the cm scale.

```{r z2y}
y <- transform2y(smocc_200, znames = "hgt_z")
all.equal(pull(y), smocc_200$hgt, tol = 0.0001)
```

We have used the Dutch 1997 height references here, but evidently similar transform could be made using other references. In practice we found that the model fit is often better when applied to $Z$-scores. Age-conditional references are common in child growth exists, but could be rare in other fields. An alternative is to apply the broken stick model to the standardized residuals of a preliminary non-linear regression of the outcome on time.
...
# cm scale
y_cm <- pull(transform2y(smocc_200, znames = "hgt_z"))
smocc_200$hgt_z <- pred
yhat_cm <- pull(transform2y(smocc_200, znames = "hgt_z"))
...
<!-- round(cor(pred, smocc_200$hgt_z, use = "complete.obs")^2, 5) -->
<!-- round(sd(pred - smocc_200$hgt_z, na.rm = TRUE), 3) -->
...
```{r tbc_trajectories_kr, echo=FALSE, fig.height=5.4, fig.cap = "Body Mass Index (BMI) SDS trajectories of six subjects, observed (blue) and fitted (red). Argyle correlation model.", set.theme = TRUE, warning=FALSE}
...
Suppose we measured the length of a boy at the ages of 1 month (52.6 cm) and 14 months (81.7 cm). The following code block uses the `centile::y2z()` function to convert the measurements to standard deviation scores (SDS) relative to the reference of the Fourth Dutch Growth Study.

```{r zscores1, echo=TRUE}
library(centile)
library(nlreferences)
ref <- load_reference("nl_1997_hgt_male_nl", pkg = "nlreferences")
boy <- data.frame(x = c(1/12, 14/12), y = c(52.6, 81.7))
boy$z <- centile::y2z(y = boy$y, x = boy$x, ref = ref)
```
