...
Object `mod_lmer` has class `lmerMod` so we may use standard `plot()`, `predict()` and `summary` methods offered by the \pkg{lme4} package. Converting the result into broken stick estimates requires summing the fixed and random effects. 
...
The calculation time of `lme4::lmer()` rapidly increases with the number of random effects. More than ten random effects (knots) takes significant time, and beyond 15 knots is generally impossible to fit. The \pkg{brokenstick} package provides another alternative, the *Kasim-Raudenbush (KR) sampler* [@kasim1998]. The method simulates draws from the posterior distributions of parameters from a two-level normal model with heterogeneous within-subject variances. The speed of the Kasim-Raudenbush sampler is almost insensitive to the number of random effects and depends primarily on the total number of iterations and somewhat on sample size. This method is available as the function `kr()` in the \pkg{brokenstick} package.
...
The call to `control_kr()` produces a list with settings for the sampler according to the conventions of the \pkg{coda} package [@plummer2006]. The defaults in `control_kr()` should be reasonable across a wide range of cases. The list component `mod_kr$mod` contains various objects of class `mcmc` with the sampling history and detailed results. For example, inspect the fixed effect estimates as
...
The `smocc_200` data contains the height measurement both in the original scale in cm (`hgt`) and the $Z$-score scale (`hgt_z`) relative to the height references from the Fourth Dutch Growth study [@fredriks2000]. For illustration, let us calculate and check height SDS using the \pkg{AGD} package.
...
The `brokenstick:::EB()` function implements the empirical Bayes (EB) estimate, also known as BLUP (@skrondal2009, p. 683). The procedure is the workhorse underlying the `predict()` method.
