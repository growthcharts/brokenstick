...
The broken stick estimates are the sum of the fixed and random effects. Note that we removed the intercept so that all predictor rows sum to 1. 

Warnings often occur when fitting broken stick models with `lme4::lmer()`. Here we have obtain the warning `Model failed to converge with max|grad| = 0.0066811 (tol = 0.002, component 1)`. I have found that we may generally ignore such warnings since the broken estimates look sound and reasonable. Note that most of my experience derives from fitting child growth data, so there is no guarantee that this apparent robustness will hold for other types of data. Warnings from `lmer()` become much more frequent for a higher number of breakpoints and larger samples sizes. One reason is that random effects in the broken stick model are highly correlated. It is kniwn that fitting the covariance matrix of the random effects is difficult for overparametrised models. In addition, very few data points inform the estimation of the random effect at the right hand boundary. As these estimates are unrealiable and uninteresting I tend to ignore the last random effect. The user may suppress warnings like these by setting `tol` to a more relaxed value. For example, adding `control = lmerControl(check.conv.grad = .makeCC("warning", tol = 1e-2))` to the arguments will silence `lmer()`. As example is

```{r}
ctl <- lmerControl(check.conv.grad = .makeCC("warning", tol = 1e-2))
fit <- lmer(f, data, control = ctl)
```

The \pkg{brokenstick} package can fit the same model in an easier way:

```{r}
mod <- brokenstick(hgt_z ~ age | id, data = smocc_200, method = "lmer",
                   knots = c(0, 0.5, 1, 2), boundary = c(0, 3),
                   control = set_control(method = "lmer",
                                         lmer = ctl))
head(predict(mod, x = "knots", shape = "wide"), 3)
```
...
The `smocc_200` data contains the height measurement both in the original scale in cm (`hgt`) and the $Z$-score scale (`hgt_z`) relative to the height references from the Fourth Dutch Growth study [@fredriks2000]. Let us  recalculate height SDS using the \pkg{AGD} package. Fortunately, we find that the same values.
...
Figure 3 shows that, as expected, the empirical Z-score distribution is close to the standard normal. The few very extremely low heights correspond to pre-term born infants. The next section concentrate on modelling `hgt_z`.

Function `z2y()` applies the inverse transformation of $Z$-scores to the original scale. The following snippet converts `hgt_z` into the cm scale.

```{r z2y}
y <- with(smocc_200, z2y(z = hgt_z, 
                         x = age, 
                         sex = ifelse(sex == "male", "M", "F"), 
                         ref = nl4.hgt))
...
```{r tbc-lmer, cache=TRUE}
ctl <- set_control(method = "lmer", 
...
```{r tbc-trajectories-lmer, fig.height=5.4, fig.cap = "Body Mass Index (BMI) SDS trajectories of six subjects, observed (blue) and fitted (red). lmer method.", set.theme = TRUE, warning=FALSE}
...
```{r tbc-trajectories-kr, echo=FALSE, fig.height=5.4, fig.cap = "Body Mass Index (BMI) SDS trajectories of six subjects, observed (blue) and fitted (red). kr method.", set.theme = TRUE, warning=FALSE}
...
```{r tbc-trajectories-all, echo=FALSE, fig.height=4, fig.cap = "Body Mass Index (BMI) SDS trajectories for 92 subjects, coloured by adult overweight status.", set.theme = TRUE, warning=FALSE}
pd <- data %>% 
  select(id, ao, `0`:`24`) %>% 
...
fit_kr <- brokenstick(hgt_z ~ age | id, data = data,
                      knots = knots, boundary = c(0, 3),
                      control = control_kr(nimp = 20, seed = 123))
