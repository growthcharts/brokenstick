...
Prediction on new data can be obtained by the `predict()` function. There are two types of prediction, which is controlled by the `at` argument. Calling `brokenstick(..., at = "knots", ...)` yields a prediction at every break age, including those in the future. Calling `brokenstick(..., age, at = "x", ...)` is more flexible, and yields a prediction at each age specified by `x`. 
...
fit_exp <- export_brokenstick(fit_hgt)
...
plot(x = d$age, y = d$hgt.z, type = "b", col = "blue", 
     ...
     xlim = c(0, 2))
brk <- c(fit_exp$knots, fit.exp$Boundary.knots[2])
...
```{r}
b <- predict(fit.exp, d$hgt.z, d$age, at = "x")
