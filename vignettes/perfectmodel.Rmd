...
library("plyr")
library("mvtnorm")
make.data.generator <- function(resid.var = 1,
                                ranef.covar = diag(c(1, 1)), n = 100
                                ) {
  ni <- nrow(ranef.covar)
  generate.data <- function() {
    # sample data set under mixed effects model with random slope/intercepts 
    simulated.data <- rdply(n, {
      b <- t(rmvnorm(n = 1, sigma = ranef.covar))
      epsilon <- rnorm(n = length(b), mean = 0, sd = sqrt(resid.var))
      b + epsilon
    })
  data.frame(
...
Let us first model the perfect situation where $\sigma^2 = 0$ (so we set `resid.var` to zero) and where the ages align perfectly.
...
gen.dat <- make.data.generator(n = 10000, 
                               ranef.covar = covar,
                               resid.var = 1)
data <- gen.dat()
head(data)
...
library("brokenstick")
knots <- 1:3
Boundary.knots <- c(1, 4)
fit <- with(data, 
            brokenstick(y = X1, x = age, subject = subject, 
                        knots = knots, Boundary.knots = Boundary.knots,
                        storeX = TRUE))
omega <- export.brokenstick(fit)$omega
beta <- export.brokenstick(fit)$beta
sigma2 <- export.brokenstick(fit)$sigma2
round(beta, 2)
round(sigma2, 4)
