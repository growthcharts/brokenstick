...
Figure 1 dispays the data from the first 500 rows as a set of growth curves of Dutch children. Curves are steeper during the first few months, so child grpwth is faster for young infants. Note also there are more cross-overs during the first half year, whereas fewer occur later. This means that the relative positions have been settled by the age of 2 years, or - put differently - that the correlation between time points at those ages is high.
...
plot(fit, new_data = data, group = ids, zband = FALSE, ylab = "Length (cm)")
```
...
plot(fit0, new_data = data, group = ids)
```
...
```{r plotfit2, cache = TRUE}
fit2 <- brokenstick(hgt.z ~ age | id, data = data, knots = 0:3)
plot(fit2, data, group = ids, x_trim = c(0, 2.2))
```
...
plot(fit9, data, group = ids, x_trim = c(0, 2.2))
```
...
The `predict_new()` function allows us to obtain various types of predictions from the broken stick model. The simplest call

```{r}
p1 <- predict_new(fit2, data)
head(p1)
...
```{r}
p2 <- predict_new(fit2, data, x = "knots")
head(p2)
nrow(p2)
```
...
```{r}
p3 <- predict_new(fit2, data, x = "knots", strip_data = FALSE)
table(p3$.source)
```
...
```{r predictatknots}
bs <- predict_new(fit9, data, x = "knots", shape = "wide")
