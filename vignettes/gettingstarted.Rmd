...
The `smocc.hgtwgt` data in the `brokenstick` package contain the heights and weights of 1933 Dutch children measured on 10 visits at ages 0-2 years. Here we take the first 2000 records from the data, corresponding to 211 different children.
...
The `fit1` object has S4 class `brokenstick`. The broken stick model contains no intercept, whereas the four predictors (`x1`-`x4`) are entered both as fixed and random effects, conditional on subject (child). The ages to which they correspond can be obtained by `get_knots(fit, include.boundaries = TRUE)`.
...
```{r}
p <- predict(fit1, x = get_knots(fit1), include.boundaries = TRUE)
head(p, 15)
```
...
```{r}
p <- predict(fit2, x = get_knots(fit.hgt), include.boundaries = TRUE)
head(p, 4)
```
...
so the nine-line broken stick model explains about `r round(100 * var(fitted(fit2)) / var(data$hgt.z))` percent of the variance of the height SDS.
...
Suppose we have four measured a new child and want to obtain predictions according to the published model stored in `export.hgt`. The following code calculates the predictions.
