...
```{r}
p2 <- predict_new(fit, data, x = "knots")
head(p2, 4)
```
...
```{r}
p3 <- predict_new(fit, data, x = "knots", strip_data = FALSE)
head(p3, 4)
```
...
plot(fit, data, group = c(10001, 10005, 10022), x_trim = c(0, 2.3))
```
...
```{r}
p1 <- predict_new(fit, data, group = 10001)
```

The `predict_new()` function can return additional information by altering these arguments. Type `?predict_new.brokenstick` at the console to see the documentation. We can plot the original data and the broken stick estimates for child 10001 jointly by

```{r fig3, eval = FALSE}
plot(fit, data, group = 10001, x_trim = c(0, 2.3))
```
...
p <- predict_new(fit, data, group = 10001, strip_data = FALSE)
...
figure(xlab = "Age (years)", ylab = "Height (cm)") %>%
  ...
  ly_lines(p$age[idx], p$yhatcm[idx], col = "orangered") %>%
  ly_points(p$age[idx], p$yhatcm[idx], col = "orangered", size = 5)
```
...
fred <- data.frame(
  ...
  id = 1)
p <- predict_new(fit, fred, x = "knots", strip_data = FALSE)
p
```
