...
## Apply to holdout sample

The `smcc_hgtwgt` data has `r nrow(smocc_hgtwgt)` rows, but until now we've used only the first 2000 rows. Is prediction equally good if we apply our fitted model to the remaining rows, that is, to observations that were not part of the model fitting? This can be calculated with the following code:
...
The correlation (in the $Z$-scale) is now equal to `r round(cor(holdout$yhat_z, holdout$haz, use = "complete.obs"), 4)`. The standard deviation of the residuals is equal to `r round(sd(holdout$yhat_z - holdout$haz, na.rm = TRUE), 3)`. In the scale of the raw meassurements we find a correlation equal to `r round(cor(holdout$yhat_cm, holdout$htcm, use = "pairwise"), 4)`. The standard deviation of the residuals in the cm scale is equal to `r round(sd(holdout$yhat_cm - holdout$htcm, na.rm = TRUE), 4)`. Such residuals  are about the size of the measurement error for height (0.5 cm), and are to be expected when applied to a new sample.
