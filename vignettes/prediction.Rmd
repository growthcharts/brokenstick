...
```{r}
UID <- unique(data$subjid)
cid <- UID[1]
d <- data[data$subjid == cid, ]
...
The scatterplot of the observed versus predicted values indicates an extremely accurate prediction. The correlation is to `r round(cor(data$yhat, data$haz, use = "complete.obs"), 4)`. The standard deviation of the residuals is equal to `r round(sd(data$yhat - data$haz, na.rm = TRUE), 3)`. 
...
The plot of the observed versus predicted is now considerably tighter, and the overall fit appears to be quite high. The correlation between the observed and predicted scores using the raw scale is equal to `r round(cor(data$yhatcm, data$lencm, use = "pairwise"), 4)`. The standard deviation of the residuals in the cm scale is equal to `r round(sd(data$yhatcm - data$lencm, na.rm = TRUE), 3)`, which is very small. 

```{r echo = FALSE}
eqscplot(y = data$yhatcm, x = data$lencm, pch = ".", ylab = "Predicted (cm)", 
...
The figure below is the scatterplot of the observed versus predicted values for the holdout sample in the Z-score scale. The correlation is to `r round(cor(data$yhat, data$haz, use = "complete.obs"), 4)`. The standard deviation of the residuals is equal to `r round(sd(data$yhat - data$haz, na.rm = TRUE), 3)`.
...
In terms of the raw meassurement scale we find a correlation between the observed and predicted scores equal to `r round(cor(data$yhatcm, data$lencm, use = "pairwise"), 4)`. The standard deviation of the residuals in the cm scale is equal to `r round(sd(data$yhatcm - data$lencm, na.rm = TRUE), 4)`, about the size of the measurement error for height.
