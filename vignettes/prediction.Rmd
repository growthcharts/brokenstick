...
fit <- with(data, brokenstick(y = hgt.z, x = age, subjid = subjid,
...
The result is stored in the `fit` object, which has S3 class `brokenstick`. It contains a component `model` with the fitted model, as well as additional attributes like `knots` and `boundary`.
...
figure(xlab = "Age (years)", ylab = "Height (SDS)", title = "Mean trajectory (n = 50)") %>%
  ly_zband(x = seq(0, 2.2, 0.2), z = -c(2.5,2,1,0)) %>%
  ly_points(get_knots(fit, "droplast"), lme4::fixef(model), hover = c(get_knots(fit), lme4::fixef(model))) %>%
  ly_lines(get_knots(fit, "droplast"), lme4::fixef(model)) %>% 
  x_range(c(-0.1, 2.3))
```
