...
This vignette assumes the `rbokeh`, `brokenstick`, `growthstandard` and `ggcorrplot` packages are installed.
...
```{r meanplot, fig.height=4, fig.width=4, fig.cap = "Mean trajectory over all children"}
figure(xlab = "Age (years)", ylab = "Height (SDS)") %>%
...
```{r traj3plot, fig.height = 3, warning=FALSE, fig.cap="Fitted trajectory of three children."}
...
```{r fig3, fig.height = 5, fig.width = 5, warning=FALSE, fig.cap="Fitted trajectory of one child."}
plot(fit, data, group = 10001, xlim = c(0, 2.1), ylim = c(-2, 2))
```
...
```{r fig.height=5, fig.width=5, fig.cap="Trajectories in CM scale."}
p <- p[p$age <= 2.5,]
...
```{r echo = TRUE, fig.height=5, fig.width=5, fig.cap="Fred's data - raw and fitted."}
plot(fit, fred, xlim = c(0, 2.1), ylim = c(-2, 2))
```
...
```{r echo = FALSE, fig.height=7, fig.width=7, fig.cap="Predicted versus observed values (SDS scale)."}
MASS::eqscplot(x = data$hgt.z, xlab = "Height (SDS)", 
...
```{r echo = FALSE, fig.width=7, fig.height = 7, fig.cap="Predicted versus observed values (CM scale)."}
