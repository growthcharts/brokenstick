...
print.brokenstick <- function(
  ...
  cat(
    "Knots       ",
    format(get_knots(x, hide = hide), digits = digits, ...),
    "\n"
  )
  cat("Mean resid  ", format(x$sigma2, digits = digits, ...), "\n")
  if (!is.null(x$r2)) {
    cat("R-squared   ", format(x$r2, digits = digits, ...), "\n")
  }
  return(invisible(x))
}

#' @export
print.summary.brokenstick <- function(
  x,
  digits = getOption("digits"),
  na.print = "",
  ...
  ...
  if (length(x$sigma2j)) {
    cat(
      "Residuals   ",
      format(x$sigma2j, digits = digits, ...),
      "(min, P25, P50, P75, max)\n"
    )
  }
  cat("Mean resid  ", format(x$sigma2, digits = digits, ...), "\n")
