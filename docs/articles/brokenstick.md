...
linear mixed model using second-order linear B-splines. The model can be
used to
...
yield the standard normal ($`Z`$) scale.
...
- *Modular*: Issues related to nonlinearities of the growth curves in
  the observed scale can be treated separately, i.e., outside the broken
  stick model;
...
data. See the [reference
page](https://growthcharts.org/brokenstick/reference/) for an overview.
